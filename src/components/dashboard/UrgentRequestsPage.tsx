            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {campaigns.filter(req => activeCategory === "All Urgent" || req.category?.name === activeCategory).map((req, i) => (
                    <UrgentRequestCard 
                        ...
                        image={req.images?.[0]?.url || `https://ui-avatars.com/api/?name=${encodeURIComponent(req.title || 'Campaign')}&background=3b82f6&color=fff&size=500&font-size=0.33&bold=true`}
