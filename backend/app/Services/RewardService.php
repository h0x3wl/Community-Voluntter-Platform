...
class RewardService
{
    ...
    private function evaluateBadges(User $user): void
    {
        ...
        $badgesToAward = [];
        if ($totalDonatedCents >= 10000) { // $100
            $badgesToAward[] = 'bronze_supporter';
        }
        if ($totalDonatedCents >= 50000) { // $500
            $badgesToAward[] = 'silver_supporter';
        }
        if ($totalDonatedCents >= 100000) { // $1000
            $badgesToAward[] = 'gold_supporter';
        }

        foreach ($badgesToAward as $code) {
            $badge = \App\Models\Badge::where('code', $code)->first();
            if ($badge) {
